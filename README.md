...
| `ogp_title` | リンク先の `og:title`（リンクプレビュー用・120グラフェム（約120字）超は切り詰め＋「…」） |
| `ogp_description` | リンク先の `og:description`（リンクプレビュー用・**80グラフェム（約80字）超は切り詰め＋「…」**） |
| `price_jpy` / `price_label` | タイトルの「¥N,NNN / N,NNN円」正規表現（>=1・金額がちょうど1つのときのみ。「編集部調べ」をタイトルに含む場合は不採用。ラベルは「発売時価格（目安）」） |
| `hex` (estimated) | `color_dict.yml` の色語ヒット時のみ（多色語は付与しない） |
| `note` | **常に空文字**（手動で追記してください） |

**記事本文の転載は行いません。** `og:title` / `og:description` はリンクプレビュー用メタデータとして収録します（og:description は80字以内に切り詰め）。出典リンク（`source_url`）と削除依頼窓口（下記「削除要請 48時間手順」・48時間以内対応）を維持します。`note` が空でなければ CI が fail します（spec23 §8）。
...
1. **追加前に robots.txt と利用規約を確認し、確認日と結論を `sources.yml` のコメントに記録**（B6）。正直UA（`CosmeDaburiBot/1.0`）で RSS URL に fetch し、403 なら追加しない。
2. `sources.yml` に新しいエントリを追加（スキーマは既存エントリを参照）。
3. `category_rules` のキーワードと `fallback_category` を設定する。
4. push後 **1週間は出力を目視確認**（誤分類・不要コンテンツの混入チェック）。
5. 問題なければ通常運用に移行。
...
- **新ソース追加時**: sources.yml のキーは `name`/`rss_url`/`require_match`/`admission_info_keywords`/`category_rules`/`fallback_category`。追加前に robots.txt と利用規約の確認を sources.yml のコメントに記録する（B6）。取得は正直UA（`CosmeDaburiBot/1.0`）のみ・403 を返すソースは追加しない（ブラウザUA偽装禁止）。手動確認は workflow_dispatch の dry_run=true で（フィードを書き換えずログだけ確認できる）
